...
/// <summary>
/// A unified functional 3-dimensional collection.
/// 
...
{
    // data
    readonly Func<int, int, int, T> GetValueByIndex;
    /// <summary>
    ...
    /// </code>
    /// </summary>
    readonly Func<IEnumerable<FunVec2<T>>> AsEnumerable;
    // data - pub
    /// <summary>
    ...
    // ctor
    FunVec3(Func<int> length1, Func<int, int> length2, Func<int, int, int> length3,
        Func<int, int, int, T> getValueByIndex,
        Opt<T> underlyingConstantValue,
        Func<IEnumerable<FunVec2<T>>> asEnumerable)
    {
        Length1 = length1;
        Length2 = length2;
        Length3 = length3;
    ...
    public FunVec3(T constantValue, Opt<int> length1 = default, Opt<Func<int, int>> getLength2 = default, Opt<Func<int, int, int>> getLength3 = default)
        : this(length1.Match(l => () => l, () => int.MaxValue),
              getLength2.UnwrapOr(FunVec2<T>.GetInfiniteLen2),
              getLength3.UnwrapOr(GetInfiniteLen3),
              (i, j, k) => constantValue, Some(constantValue),
              () => Enumerable.Range(0, length1.UnwrapOr(int.MaxValue))
                    .Select(i => new FunVec2<T>(constantValue, Some(getLength2.UnwrapOr(FunVec2<T>.GetInfiniteLen2)(i)))))
    { }
    /// <summary>
    ...
    /// <param name="getLength2">Optional function (i -> length) to get length of the i-th collection; when omitted will default to None which will yield to a length of int.MaxValue for any index.</param>
    /// <param name="getLength3">Optional function (i,j -> length) to get length of the (i,j)-th collection; when omitted will default to None which will yield to a length of int.MaxValue for any indices.</param>
    public FunVec3(Func<int, int, int, T> getValue, Opt<int> length1 = default, Opt<Func<int, int>> getLength2 = default,
        ...
        : this(length1.Match(l => () => l, () => int.MaxValue),
              ...
              getValue, None<T>(),
              () => Enumerable.Range(0, length1.UnwrapOr(int.MaxValue))
                    .Select(i => new FunVec2<T>((j, k) => getValue(i, j, k), Some(getLength2.UnwrapOr(FunVec2<T>.GetInfiniteLen2)(i)))))
    { }
    /// <summary>
    ...
    public FunVec3(T[][][] array)
        : this(() => array.Length,
              i => array[i].Length,
              (i, j) => array[i][j].Length,
              (i, j, k) => array[i][j][k], None<T>(),
              () => Enumerable.Range(0, array.Length)
                    .Select(i => new FunVec2<T>(array[i])))
    { }
    /// <summary>
    ...
    public FunVec3(List<List<List<T>>> list)
        : this(() => list.Count,
              i => list[i].Count,
              (i, j) => list[i][j].Count,
              (i, j, k) => list[i][j][k], None<T>(),
              () => Enumerable.Range(0, list.Count)
                    .Select(i => new FunVec2<T>(list[i])))
    { }
    // helper
    internal static int GetInfiniteLen3(int _i, int _k)
        => int.MaxValue;
    ...
    public Opt<T> Get(int i, int j, int k)
    {
        if (i >= 0 && i < Length1() && j >= 0 && j < Length2(i) && k >= 0 && k < Length3(i, j))
            return Some(GetValueByIndex(i, j, k));
        else
            return None<T>();
    }
    /// <summary>
    ...
    /// </summary>
    /// <returns></returns>
    public IEnumerator<FunVec2<T>> GetEnumerator()
    ...
    // implicit
    /// <summary>
    /// <inheritdoc cref="FunVec3{T}.FunVec3(T, Opt{int}, Opt{Func{int, int}}, Opt{Func{int, int, int}})"/>
    /// </summary>
    /// <param name="constantValue"></param>
    public static implicit operator FunVec3<T>(T constantValue) => new(constantValue);
    /// <summary>
    /// <inheritdoc cref="FunVec3{T}.FunVec3(Func{int, int, int, T}, Opt{int}, Opt{Func{int, int}}, Opt{Func{int, int, int}})"/>
    /// </summary>
    /// <param name="getValueByIndex"></param>
    public static implicit operator FunVec3<T>(Func<int, int, int, T> getValueByIndex) => new(getValueByIndex);
    /// <summary>
    /// <inheritdoc cref="FunVec3{T}.FunVec3(T[][][])"/>
    /// </summary>
    /// <param name="array"></param>
    public static implicit operator FunVec3<T>(T[][][] array) => new(array);
    /// <summary>
    /// <inheritdoc cref="FunVec3{T}.FunVec3(List{List{List{T}}})"/>
    /// </summary>
    /// <param name="list"></param>
    public static implicit operator FunVec3<T>(List<List<List<T>>> list) => new(list);
}
